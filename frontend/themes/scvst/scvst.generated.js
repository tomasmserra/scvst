...
window.Vaadin.Flow['_vaadintheme_scvst_globalCss'] = window.Vaadin.Flow['_vaadintheme_scvst_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_scvst_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_scvst_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_scvst_globalCss'].push(target);
  }
  if (!document['_vaadintheme_scvst_componentCss']) {
    
    document['_vaadintheme_scvst_componentCss'] = true;
  }
